...
library(stringi)      # to parse numbers from JSON and TTR
library(curl)
library(xts)
library(TTR)

# Functions:
curr_bal_cad <- function(x) {
...
sell_exe <- function(x) {
  # place initial order
  while (curr_bal_eth() > 0) {
    add_order("ETH-CAD", api.key = "api_key", secret = "secret", passphrase = "passphrase", type = "limit", price = ask(), side = "s", size = curr_bal_eth())
    # sleep to see if order takes
    Sys.sleep(17)
    # check to see if ETH bal >= order amt
    if (curr_bal_eth() == 0) {
...
# Actual Trading Loop
if (curr_bal_cad() >= 20) {        # if have more than $20 CAD start loop
